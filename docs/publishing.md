...
GitHub Pages hosting requires no VPS or Caddy changes. For MiniZap VPS hosting, see [deployment instructions](../deploy/README.md). The custom Pages domain is `potato.minizap.online`; online requests go to `api.minizap.online` on the VPS. Short URLs use `/?flight=<id>`, which works without server rewrites and returns HTTP 200 from Pages. Legacy long hash links still work.
