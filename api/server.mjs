...
import { validateGameRules } from './replay-policy.mjs';
...
export function createApi({ dbPath = ':memory:', gameOrigin = 'https://potato.minizap.online', origins = [gameOrigin], trustProxy = false, rateLimit = 120, verifyTimeout = 5000, maxFlights = 25000, maxDatabaseBytes = 64 * 1024 * 1024, maxUploads = 32, bodyTimeout = 10000 } = {}) {
  ...
  db.exec(`PRAGMA max_page_count=${Math.max(1, Math.floor(maxDatabaseBytes / pageSize))}`);
  let flightCount = db.prepare('SELECT count(*) AS count FROM flights').get().count;
  let uploads = 0;
  ...
  async function verify(replay) {
    ...
    try {
      return await new Promise((resolve, reject) => {
        worker = new Worker(new URL('./verify-worker.mjs', import.meta.url), { workerData: replay, resourceLimits: { maxOldGenerationSizeMb: 64, stackSizeMb: 4 } });
        workers.add(worker);
        timer = setTimeout(() => reject(fail(422, 'verification_timeout', 'Flugprüfung hat zu lange gedauert.')), verifyTimeout);
        ...
        worker.once('exit', () => reject(fail(500, 'verification_failed', 'Flugprüfung wurde beendet.')));
      });
    } finally {
      clearTimeout(timer);
      // Hold the concurrency slot until the worker has actually exited, including timeouts.
      if (worker) { await worker.terminate(); workers.delete(worker); }
      active--;
    }
  }
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      const ip = trustProxy && localProxy && isIP(forwarded) ? forwarded : peer;
      if (!clients.has(ip)) {
      ...
      if (req.method === 'POST' && path === `${PREFIX}/flights`) {
        if (uploads >= maxUploads) throw fail(503, 'busy', 'Bitte gleich noch einmal versuchen.');
        uploads++; upload = true;
        ...
        const timer = setTimeout(() => req.destroy(), bodyTimeout);
        ...
        let body, replay;
        try { body = JSON.parse(Buffer.concat(chunks).toString()); } catch { throw fail(422, 'invalid_replay', 'Ungültige JSON-Flugdaten.'); }
        try { if (!body || typeof body.listed !== 'boolean') throw Error('Sichtbarkeit fehlt.'); replay = validateReplay(body.replay); validateGameRules(replay); }
        ...
        if (!row) {
          if (flightCount >= maxFlights) throw fail(503, 'storage_full', 'Der Flugspeicher ist voll. Bitte später versuchen.');
      ...
      if (error.errcode === 13) error = fail(503, 'storage_full', 'Der Flugspeicher ist voll. Bitte später versuchen.');
      send(error.status || 500, { error: error.code || 'internal_error', message: error.status ? error.message : 'Serverfehler.' });
